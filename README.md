...
This means we can learn the patterns of one person and then detect whether the current user is this person or not.
...
After training the models, we safe them and then load them in the file `raltimeModel.py`.
Then we use the pretrained model to detect anomalies on the incoming data. We collect the live data similar to the data collection step but instead of writing it into a CSV file, we collect it in a buffer and then pass it to the data pre-processing and then to the model to predict possbile anomalies.
